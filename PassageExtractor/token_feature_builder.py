...
def greek(token):
    p = re.compile(
        "alpha|beta|gamma|delta|epsilon|zeta|theta|iota|kappa|lambda|omicron|rho|sigma|tau|upsilon|phi|psi|omega|chi|nu|pi|xi|eta|mu")
    if p.match(token) is not None:
        return 1
    else:
        return 0
...
def units(token):
    p = re.compile(
        "mkhz|khz|hz|kg|g|mg|ng|km|m|cm|mm|nm|ng|kb|l|ml|nl|mol|j|w|v|n|k|a|cal|min|h|s|db|cd|d|bp|c|sec|mm|mmhg|pg|ppm|meq|liter|cu|cfu|mci|kj")
    if p.match(token) is not None:
        return 1
    else:
        return 0
...
def amino_acid(token):
    p = re.compile(
        "(isoleucine|alanine|leucine|asparagine|lysine|aspartate|methionine|cysteine|phenylalanine|glutamate|threonine|glutamine|tryptophan|glycine|valine|proline|arginine|serine|histidine|tyrosine)[s]*")
    if p.match(token) is not None:
        return 1
    else:
        return 0
...
def vitamin(token):
    p = re.compile(
        "retinoids|retinol|carotenoids|thiamine|riboflavin|niacin|niacinamide|pantothenic|pyridoxine|pyridoxamine|pyridoxal|biotin|folic|folinic|cyanocobalamin|hydroxycobalamin|methylcobalamin|ascorbic|ergocalciferol|cholecalciferol|tocopherols|tocotrienols|phylloquinone|menaquinones")
    if p.match(token) is not None:
        return 1
    else:
        return 0
...
def features_for_EFO(root, token):
    """
